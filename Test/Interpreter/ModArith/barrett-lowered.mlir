...
// RUN: veir-opt %s -p=mod-arith-to-arith-barrett > %t.lowered.mlir && veir-interpret %t.lowered.mlir | filecheck %s
// RUN: veir-opt %s -p="mod-arith-to-arith-barrett,reconcile-cast" > %t.reconciled.mlir && veir-interpret %t.reconciled.mlir | filecheck %s

// Differential test for the Barrett-based lowering: interpreting the program before and
// after lowering (and after cast reconciliation) must produce the same output.
//
// Computes ((250 + 10) * 250 - 250) mod 251 = (9 * 250 - 250) mod 251 = 243.
