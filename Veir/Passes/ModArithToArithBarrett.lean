...
def addBarrettRecipe (lhs rhs : ValuePtr) (mt : ModArithType) : List OpDescr :=
  ...
  [ castDescr (.outer lhs) storageTy,            -- 0: lhs as iN
    ...
    binopDescr .addi { attr := { nsw := false, nuw := false } } (.created 0 0) (.created 1 0) n,
                                                 -- 2: t = x + y < 2q
    constantDescr mt.modulus.value n,            -- 3: q
    cmpiUgeDescr (.created 2 0) (.created 3 0),  -- 4: t ≥ q
    binopDescr .subi { attr := { nsw := false, nuw := false } } (.created 2 0) (.created 3 0) n,
                                                 -- 5: t - q
...
def subBarrettRecipe (lhs rhs : ValuePtr) (mt : ModArithType) : List OpDescr :=
  ...
  [ castDescr (.outer lhs) storageTy,            -- 0: lhs as iN
    ...
    binopDescr .addi { attr := { nsw := false, nuw := false } } (.created 0 0) (.created 2 0) n,
                                                 -- 3: x + q < 2q
    binopDescr .subi { attr := { nsw := false, nuw := false } } (.created 3 0) (.created 1 0) n,
                                                 -- 4: t = x + q - y ∈ (0, 2q)
    cmpiUgeDescr (.created 4 0) (.created 2 0),  -- 5: t ≥ q
    binopDescr .subi { attr := { nsw := false, nuw := false } } (.created 4 0) (.created 2 0) n,
                                                 -- 6: t - q
...
def mulBarrettRecipe (lhs rhs : ValuePtr) (mt : ModArithType) : List OpDescr :=
  ...
  [ castDescr (.outer lhs) storageTy,            -- 0: lhs as iN
    ...
    binopDescr .muli { attr := { nsw := false, nuw := false } } (.created 1 0) (.created 3 0) w,
                                                 -- 4: p = x·y < q²
    constantDescr ratio w,                       -- 5: r = ⌊2^(2N)/q⌋
    binopDescr .muli { attr := { nsw := false, nuw := false } } (.created 4 0) (.created 5 0) w,
                                                 -- 6: p·r < 2^(4N-2)
    ...
    binopDescr .muli { attr := { nsw := false, nuw := false } } (.created 8 0) (.created 9 0) w,
                                                 -- 10: s·q ≤ p
    binopDescr .subi { attr := { nsw := false, nuw := false } } (.created 4 0) (.created 10 0) w,
                                                 -- 11: t = p - s·q ∈ [0, 2q)
    cmpiUgeDescr (.created 11 0) (.created 9 0), -- 12: t ≥ q
    binopDescr .subi { attr := { nsw := false, nuw := false } } (.created 11 0) (.created 9 0) w,
                                                 -- 13: t - q
