...
def interpretOp' (ctx : IRContext) (opPtr : OperationPtr) (operands: Array RuntimeValue)
  ...
  | .arith_constant => do
    ...
    let .integerAttr intAttr := value.value
      | none
    let res ← op.results[0]?
    let .integerType bw := res.type.val
      | none
    return (#[.int bw.bitwidth
      (.val (BitVec.ofNat bw.bitwidth intAttr.value.toNat))], .continue)
  | .arith_addi => do
  ...
  | .mod_arith_constant => do
    ...
    let .integerAttr intAttr := valueProp.value
      | none
    let value ← normalizeMod? modType.modulus intAttr.value
